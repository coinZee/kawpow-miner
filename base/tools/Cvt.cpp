...
bool jdkcat::Cvt::fromHex(Buffer &buf, const char *in, size_t size)
{
    return fromHexImpl(buf, in, size);
}


bool jdkcat::Cvt::fromHex(Buffer &buf, const rapidjson::Value &value)
{
    if (!value.IsString()) {
        return false;
    }
...
bool jdkcat::Cvt::fromHex(std::string &buf, const char *in, size_t size)
{
    return fromHexImpl(buf, in, size);
}


bool jdkcat::Cvt::fromHex(uint8_t *bin, size_t bin_maxlen, const char *hex, size_t hex_len)
{
...
bool jdkcat::Cvt::fromHex(uint8_t *bin, size_t bin_maxlen, const rapidjson::Value &value)
{
    if (!value.IsString()) {
        return false;
    }
...
jdkcat::Buffer jdkcat::Cvt::fromHex(const char *in, size_t size)
{
    Buffer buf;
...
bool jdkcat::Cvt::toHex(char *hex, size_t hex_maxlen, const uint8_t *bin, size_t bin_len)
{
    return cvt_bin2hex(hex, hex_maxlen, bin, bin_len) != nullptr;
}


jdkcat::Buffer jdkcat::Cvt::randomBytes(const size_t size)
{
    Buffer buf(size);
...
rapidjson::Value jdkcat::Cvt::toHex(const Buffer &data, rapidjson::Document &doc)
{
    return toHex(data.data(), data.size(), doc);
}


rapidjson::Value jdkcat::Cvt::toHex(const Span &data, rapidjson::Document &doc)
{
    return toHex(data.data(), data.size(), doc);
}


rapidjson::Value jdkcat::Cvt::toHex(const std::string &data, rapidjson::Document &doc)
{
    return toHex(reinterpret_cast<const uint8_t *>(data.data()), data.size(), doc);
}


rapidjson::Value jdkcat::Cvt::toHex(const uint8_t *in, size_t size, rapidjson::Document &doc)
{
    return toHex(in, size).toJSON(doc);
}


jdkcat::String jdkcat::Cvt::toHex(const uint8_t *in, size_t size)
{
    assert(in != nullptr && size > 0);
    if (in == nullptr || size == 0) {
        return {};
...
void jdkcat::Cvt::randomBytes(void *buf, size_t size)
{
#   ifndef jdkcat_SODIUM
    std::uniform_int_distribution<> dis(0, 255);
