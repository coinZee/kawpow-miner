...
jdkcat::Arguments::Arguments(int argc, char **argv) :
    m_argv(argv),
    m_argc(argc)
{
...
bool jdkcat::Arguments::hasArg(const char *name) const
{
    if (m_argc == 1) {
        return false;
    }
...
const char *jdkcat::Arguments::value(const char *key1, const char *key2) const
{
...
void jdkcat::Arguments::add(const char *arg)
{
    if (arg == nullptr) {
        return;
    }
