...
jdkcat::Algorithm::Algorithm(const rapidjson::Value &value) :
    m_id(parse(value.GetString()))
{
}


jdkcat::Algorithm::Algorithm(uint32_t id) :
    m_id(kAlgorithmNames.count(id) ? static_cast<Id>(id) : INVALID)
{
}


const char *jdkcat::Algorithm::name() const
{
    if (!isValid()) {
        return kINVALID;
    }
...
rapidjson::Value jdkcat::Algorithm::toJSON() const
{
...
rapidjson::Value jdkcat::Algorithm::toJSON(rapidjson::Document &) const
{
    return toJSON();
}


jdkcat::Algorithm::Id jdkcat::Algorithm::parse(const char *name)
{
...
size_t jdkcat::Algorithm::count()
{
    return kAlgorithmNames.size();
}


std::vector<jdkcat::Algorithm> jdkcat::Algorithm::all(const std::function<bool(const Algorithm &algo)> &filter)
{
