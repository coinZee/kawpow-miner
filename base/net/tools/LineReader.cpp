...
jdkcat::LineReader::~LineReader()
{
    NetBuffer::release(m_buf);
}


void jdkcat::LineReader::parse(char *data, size_t size)
{
...
void jdkcat::LineReader::add(const char *data, size_t size)
{
    if (size + m_pos > jdkcat_NET_BUFFER_CHUNK_SIZE) {
...
void jdkcat::LineReader::getline(char *data, size_t size)
{
    char *end        = nullptr;
    char *start      = data;
