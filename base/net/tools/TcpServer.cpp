...
static const jdkcat::String kLocalHost("127.0.0.1");


jdkcat::TcpServer::TcpServer(const String &host, uint16_t port, ITcpServerListener *listener) :
...
jdkcat::TcpServer::~TcpServer()
{
    Handle::close(m_tcp);
}


int jdkcat::TcpServer::bind()
{
    if (!m_version) {
...
void jdkcat::TcpServer::create(uv_stream_t *stream, int status)
{
    if (status < 0) {
        return;
    }
...
void jdkcat::TcpServer::onConnection(uv_stream_t *stream, int status)
{
