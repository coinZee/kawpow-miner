...
jdkcat::HttpsClient::HttpsClient(const char *tag, FetchRequest &&req, const std::weak_ptr<IHttpListener> &listener) :
...
jdkcat::HttpsClient::~HttpsClient()
{
    if (m_ctx) {
        SSL_CTX_free(m_ctx);
    }
...
const char *jdkcat::HttpsClient::tlsFingerprint() const
{
    return m_ready ? m_fingerprint : nullptr;
}


const char *jdkcat::HttpsClient::tlsVersion() const
{
    return m_ready ? SSL_get_version(m_ssl) : nullptr;
}


void jdkcat::HttpsClient::handshake()
{
    m_ssl = SSL_new(m_ctx);
...
void jdkcat::HttpsClient::read(const char *data, size_t size)
{
...
void jdkcat::HttpsClient::write(std::string &&data, bool close)
{
...
bool jdkcat::HttpsClient::verify(X509 *cert)
{
    if (cert == nullptr) {
        return false;
    }
...
bool jdkcat::HttpsClient::verifyFingerprint(X509 *cert)
{
...
void jdkcat::HttpsClient::flush(bool close)
{
