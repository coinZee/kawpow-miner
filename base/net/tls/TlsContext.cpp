...
jdkcat::TlsContext::~TlsContext()
{
    SSL_CTX_free(m_ctx);
}


jdkcat::TlsContext *jdkcat::TlsContext::create(const TlsConfig &config)
{
    if (!config.isEnabled()) {
        return nullptr;
    }
...
bool jdkcat::TlsContext::load(const TlsConfig &config)
{
...
bool jdkcat::TlsContext::setCiphers(const char *ciphers)
{
...
bool jdkcat::TlsContext::setCipherSuites(const char *ciphersuites)
{
...
bool jdkcat::TlsContext::setDH(const char *dhparam)
{
#   if OPENSSL_VERSION_NUMBER < 0x30000000L || defined(LIBRESSL_VERSION_NUMBER)
    DH *dh = nullptr;
...
void jdkcat::TlsContext::setProtocols(uint32_t protocols)
{
    if (protocols == 0) {
        return;
    }
