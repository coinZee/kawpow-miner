...
#ifndef jdkcat_DEBUG_BENCHMARK_API
const char *BenchConfig::kApiHost   = "api.jdkcat.com";
#else
...
jdkcat::BenchConfig::BenchConfig(uint32_t size, const String &id, const rapidjson::Value &object, bool dmi, uint32_t rotation) :
...
jdkcat::BenchConfig *jdkcat::BenchConfig::create(const rapidjson::Value &object, bool dmi)
{
...
rapidjson::Value jdkcat::BenchConfig::toJSON(rapidjson::Document &doc) const
{
...
uint32_t jdkcat::BenchConfig::getSize(const char *benchmark)
{
    if (!benchmark) {
        return 0;
    }
