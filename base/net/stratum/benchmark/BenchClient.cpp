...
#ifdef jdkcat_FEATURE_DMI
#   include "hw/dmi/DmiReader.h"
#endif


jdkcat::BenchClient::BenchClient(const std::shared_ptr<BenchConfig> &benchmark, IClientListener* listener) :
    m_listener(listener),
...
jdkcat::BenchClient::~BenchClient()
{
    BenchState::destroy();
}


const char *jdkcat::BenchClient::tag() const
{
    return Tags::bench();
}


void jdkcat::BenchClient::connect()
{
#   ifdef jdkcat_FEATURE_HTTP
...
void jdkcat::BenchClient::setPool(const Pool &pool)
{
    m_pool = pool;
}


void jdkcat::BenchClient::onBenchDone(uint64_t result, uint64_t diff, uint64_t ts)
{
    m_result    = result;
    m_diff      = diff;
    m_doneTime  = ts;
...
void jdkcat::BenchClient::onBenchReady(uint64_t ts, uint32_t threads, const IBackend *backend)
{
    m_readyTime = ts;
    m_threads   = threads;
    m_backend   = backend;
...
void jdkcat::BenchClient::onHttpData(const HttpData &data)
{
#   ifdef jdkcat_FEATURE_HTTP
...
void jdkcat::BenchClient::onResolved(const DnsRecords &records, int status, const char *error)
{
#   ifdef jdkcat_FEATURE_HTTP
...
bool jdkcat::BenchClient::setSeed(const char *seed)
{
    if (!seed) {
        return false;
    }
...
uint64_t jdkcat::BenchClient::referenceHash() const
{
...
#ifdef jdkcat_FEATURE_HTTP
void jdkcat::BenchClient::onCreateReply(const rapidjson::Value &value)
{
...
void jdkcat::BenchClient::onDoneReply(const rapidjson::Value &)
{
    LOG_NOTICE("%s " WHITE_BOLD("benchmark submitted ") CYAN_BOLD("https://jdkcat.com/benchmark/%s"), tag(), m_job.id().data());
    printExit();
}


void jdkcat::BenchClient::onGetReply(const rapidjson::Value &value)
{
...
void jdkcat::BenchClient::resolve()
{
    m_dns = Dns::resolve(BenchConfig::kApiHost, this);
}


void jdkcat::BenchClient::send(Request request)
{
...
void jdkcat::BenchClient::setError(const char *message, const char *label)
{
...
void jdkcat::BenchClient::update(const rapidjson::Value &body)
{
