...
jdkcat::NetworkState::NetworkState(IStrategyListener *listener) : StrategyProxy(listener)
{
}


#ifdef jdkcat_FEATURE_API
rapidjson::Value jdkcat::NetworkState::getConnection(rapidjson::Document &doc, int version) const
{
...
rapidjson::Value jdkcat::NetworkState::getResults(rapidjson::Document &doc, int version) const
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
void jdkcat::NetworkState::printConnection() const
{
    if (!m_active) {
...
void jdkcat::NetworkState::printResults() const
{
    if (!m_hashes) {
...
const char *jdkcat::NetworkState::scaleDiff(uint64_t &diff)
{
...
std::string jdkcat::NetworkState::humanDiff(uint64_t diff)
{
...
void jdkcat::NetworkState::onActive(IStrategy *strategy, IClient *client)
{
...
void jdkcat::NetworkState::onJob(IStrategy *strategy, IClient *client, const Job &job, const rapidjson::Value &params)
{
...
void jdkcat::NetworkState::onPause(IStrategy *strategy)
{
...
void jdkcat::NetworkState::onResultAccepted(IStrategy *strategy, IClient *client, const SubmitResult &result, const char *error)
{
    add(result, error);
...
uint32_t jdkcat::NetworkState::latency() const
{
...
uint64_t jdkcat::NetworkState::avgTime() const
{
    if (m_latency.empty()) {
        return 0;
    }
...
uint64_t jdkcat::NetworkState::connectionTime() const
{
    return m_active ? ((Chrono::steadyMSecs() - m_connectionTime)) : 0;
}


void jdkcat::NetworkState::add(const SubmitResult &result, const char *error)
{
    if (error) {
        m_rejected++;
        return;
