...
jdkcat::SelfSelectClient::SelfSelectClient(int id, const char *agent, IClientListener *listener, bool submitToOrigin) :
...
jdkcat::SelfSelectClient::~SelfSelectClient()
{
    delete m_client;
}


int64_t jdkcat::SelfSelectClient::submit(const JobResult &result)
{
    if (m_submitToOrigin) {
...
void jdkcat::SelfSelectClient::tick(uint64_t now)
{
    m_client->tick(now);
...
void jdkcat::SelfSelectClient::onJobReceived(IClient *, const Job &job, const rapidjson::Value &)
{
    m_job = job;
...
void jdkcat::SelfSelectClient::onLogin(IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
...
bool jdkcat::SelfSelectClient::parseResponse(int64_t id, rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == -1) {
        return false;
    }
...
void jdkcat::SelfSelectClient::getBlockTemplate()
{
    setState(WaitState);
...
void jdkcat::SelfSelectClient::retry()
{
    setState(RetryState);
}


void jdkcat::SelfSelectClient::setState(State state)
{
    if (m_state == state) {
        return;
    }
...
void jdkcat::SelfSelectClient::submitBlockTemplate(rapidjson::Value &result)
{
    using namespace rapidjson;
    Document doc(kObjectType);
    auto &allocator = doc.GetAllocator();
...
void jdkcat::SelfSelectClient::submitOriginDaemon(const JobResult& result)
{
...
void jdkcat::SelfSelectClient::onHttpData(const HttpData &data)
{
    if (data.status != 200) {
        return retry();
    }
