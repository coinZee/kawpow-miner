...
#if defined jdkcat_ALGO_KAWPOW || defined jdkcat_ALGO_GHOSTRIDER
...
jdkcat::Pool::Pool(const char *url) :
...
jdkcat::Pool::Pool(const char *host, uint16_t port, const char *user, const char *password, const char* spendSecretKey, int keepAlive, bool nicehash, bool tls, Mode mode) :
...
jdkcat::Pool::Pool(const rapidjson::Value &object) :
    m_flags(1 << FLAG_ENABLED),
    m_pollInterval(kDefaultPollInterval),
    m_jobTimeout(kDefaultJobTimeout),
...
#ifdef jdkcat_FEATURE_BENCHMARK
jdkcat::Pool::Pool(const std::shared_ptr<BenchConfig> &benchmark) :
...
jdkcat::BenchConfig *jdkcat::Pool::benchmark() const
{
...
uint32_t jdkcat::Pool::benchSize() const
{
...
bool jdkcat::Pool::isEnabled() const
{
#   ifndef jdkcat_FEATURE_TLS
    if (isTLS()) {
        return false;
    }
#   endif

#   ifndef jdkcat_FEATURE_HTTP
...
bool jdkcat::Pool::isEqual(const Pool &other) const
{
...
jdkcat::IClient *jdkcat::Pool::createClient(int id, IClientListener *listener) const
{
...
rapidjson::Value jdkcat::Pool::toJSON(rapidjson::Document &doc) const
{
...
std::string jdkcat::Pool::printableName() const
{
...
void jdkcat::Pool::setKeepAlive(const rapidjson::Value &value)
{
    if (value.IsInt()) {
