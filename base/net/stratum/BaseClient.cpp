...
jdkcat::BaseClient::BaseClient(int id, IClientListener *listener) :
    m_listener(listener),
    m_id(id)
{
}
...
void jdkcat::BaseClient::setPool(const Pool &pool)
{
    if (!pool.isValid()) {
        return;
    }
...
bool jdkcat::BaseClient::handleResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == 1) {
        return false;
    }
...
bool jdkcat::BaseClient::handleSubmitResponse(int64_t id, const char *error)
{
