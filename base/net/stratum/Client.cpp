 ...
 * Copyright (c) 2016-2024 jdkcat       <https://github.com/jdkcat>, <support@jdkcat.com>
 *
...
jdkcat::Client::Client(int id, const char *agent, IClientListener *listener) :
...
jdkcat::Client::~Client()
{
    delete m_socket;
}


bool jdkcat::Client::disconnect()
{
    m_keepAlive = 0;
    m_expire    = 0;
    m_failures  = -1;
...
bool jdkcat::Client::isTLS() const
{
#   ifdef jdkcat_FEATURE_TLS
...
const char *jdkcat::Client::tlsFingerprint() const
{
#   ifdef jdkcat_FEATURE_TLS
...
const char *jdkcat::Client::tlsVersion() const
{
#   ifdef jdkcat_FEATURE_TLS
    if (isTLS()) {
...
int64_t jdkcat::Client::send(const rapidjson::Value &obj, Callback callback)
{
...
int64_t jdkcat::Client::send(const rapidjson::Value &obj)
{
...
int64_t jdkcat::Client::submit(const JobResult &result)
{
...
void jdkcat::Client::connect(const Pool &pool)
{
    setPool(pool);
    connect();
}
...
void jdkcat::Client::deleteLater()
{
    if (!m_listener) {
        return;
    }
...
void jdkcat::Client::tick(uint64_t now)
{
...
void jdkcat::Client::onResolved(const DnsRecords &records, int status, const char *error)
{
    m_dns.reset();
...
bool jdkcat::Client::parseJob(const rapidjson::Value &params, int *code)
{
    if (!params.IsObject()) {
        *code = 2;
        return false;
...
#   endif
    ...
    if (m_jobs == 0) { // https://github.com/jdkcat/jdkcat/issues/459
        return false;
    }
...
bool jdkcat::Client::send(BIO *bio)
{
#   ifdef jdkcat_FEATURE_TLS
    uv_buf_t buf;
...
bool jdkcat::Client::verifyAlgorithm(const Algorithm &algorithm, const char *algo) const
{
...
bool jdkcat::Client::write(const uv_buf_t &buf)
{
...
int jdkcat::Client::resolve(const String &host)
{
...
int64_t jdkcat::Client::send(size_t size)
{
...
void jdkcat::Client::connect(const sockaddr *addr)
{
...
bool jdkcat::Client::parseLogin(const rapidjson::Value &result, int *code)
{
...
void jdkcat::Client::parse(char *line, size_t len)
{
    startTimeout();
...
void jdkcat::Client::parseExtensions(const rapidjson::Value &result)
{
    m_extensions.reset();
...
void jdkcat::Client::parseNotification(const char *method, const rapidjson::Value &params, const rapidjson::Value &)
{
...
void jdkcat::Client::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
...
void jdkcat::Client::read(ssize_t nread, const uv_buf_t *buf)
{
...
void jdkcat::Client::setState(SocketState state)
{
...
void jdkcat::Client::startTimeout()
{
    m_expire = 0;
...
bool jdkcat::Client::isCriticalError(const char *message)
{
    if (!message) {
        return false;
    }
...
void jdkcat::Client::onClose(uv_handle_t *handle)
{
...
void jdkcat::Client::onConnect(uv_connect_t *req, int status)
{
...
void jdkcat::Client::onRead(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
