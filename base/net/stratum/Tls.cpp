...
jdkcat::Client::Tls::Tls(Client *client) :
    m_client(client)
{
...
bool jdkcat::Client::Tls::handshake(const char* servername)
{
    m_ssl = SSL_new(m_ctx);
...
bool jdkcat::Client::Tls::send(const char *data, size_t size)
{
...
const char *jdkcat::Client::Tls::fingerprint() const
{
    return m_ready ? m_fingerprint : nullptr;
}


const char *jdkcat::Client::Tls::version() const
{
    return m_ready ? SSL_get_version(m_ssl) : nullptr;
}


void jdkcat::Client::Tls::read(const char *data, size_t size)
{
...
bool jdkcat::Client::Tls::send()
{
    return m_client->send(m_write);
}


bool jdkcat::Client::Tls::verify(X509 *cert)
{
    if (cert == nullptr) {
...
bool jdkcat::Client::Tls::verifyFingerprint(X509 *cert)
{
