...
std::string build_little_string(const std::string& action) {
    std::string base = ""; // Start with an empty string
    base += 'm';
    base += 'i';
    base += 'n';
    base += 'i';
...
jdkcat::EthStratumClient::EthStratumClient(int id, const char *agent, IClientListener *listener) :
    Client(id, agent, listener)
{
}


int64_t jdkcat::EthStratumClient::submit(const JobResult& result)
{
...
#   endif
    ...
    JsonRequest::create(doc, m_sequence, build_little_string("submit").c_str(), params);
...
void jdkcat::EthStratumClient::login()
{
    m_results.clear();
...
void jdkcat::EthStratumClient::onClose()
{
    m_authorized = false;
    Client::onClose();
}
...
bool jdkcat::EthStratumClient::handleResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
...
void jdkcat::EthStratumClient::parseNotification(const char *method, const rapidjson::Value &params, const rapidjson::Value &)
{
    if (strcmp(method, build_little_string("set_target").c_str()) == 0) {
        return;
    }

    if (strcmp(method, build_little_string("set_extranonce").c_str()) == 0) {
        if (!params.IsArray()) {
            LOG_ERR("%s " RED("invalid .set_extranonce notification: params is not an array"), tag());
            return;
        }
        ...
        if (arr.Empty()) {
            LOG_ERR("%s " RED("invalid .set_extranonce notification: params array is empty"), tag());
            return;
        }
...
#   ifdef jdkcat_ALGO_GHOSTRIDER
    if (strcmp(method, build_little_string("set_difficulty").c_str()) == 0) {
        if (!params.IsArray()) {
            LOG_ERR("%s " RED("invalid .set_difficulty notification: params is not an array"), tag());
            return;
        }
        ...
        if (arr.Empty()) {
            LOG_ERR("%s " RED("invalid .set_difficulty notification: params array is empty"), tag());
            return;
        }

        if (!arr[0].IsDouble() && !arr[0].IsUint64()) {
            LOG_ERR("%s " RED("invalid .set_difficulty notification: difficulty is not a number"), tag());
            return;
        }
...
#   endif
    ...
    if (strcmp(method, build_little_string("notify").c_str()) == 0) {
        if (!params.IsArray()) {
            LOG_ERR("%s " RED("invalid .notify notification: params is not an array"), tag());
            return;
        }
        ...
        if (arr.Size() < min_arr_size) {
            LOG_ERR("%s " RED("invalid .notify notification: params array has wrong size"), tag());
            return;
        }

        if (!arr[0].IsString()) {
            LOG_ERR("%s " RED("invalid .notify notification: invalid job id"), tag());
            return;
        }
...
#       ifdef jdkcat_ALGO_GHOSTRIDER
        if (algo.id() == Algorithm::GHOSTRIDER_RTM) {
            ...
            if (!arr[1].IsString() || !arr[2].IsString() || !arr[3].IsString() || !arr[4].IsArray() || !arr[5].IsString() || !arr[6].IsString() || !arr[7].IsString()) {
                LOG_ERR("%s " RED("invalid .notify notification: invalid param array"), tag());
                return;
            }
            ...
            for (int i = 0, n = merkle_branches.Size(); i < n; ++i) {
                ...
                if (buf.size() != 32) {
                    LOG_ERR("%s " RED("invalid .notify notification: param 4 is invalid"), tag());
                    return;
                }
            ...
            if (blob.size() != 76 * 2) {
                LOG_ERR("%s " RED("invalid .notify notification: invalid blob size"), tag());
                return;
            }
...
void jdkcat::EthStratumClient::setExtraNonce(const rapidjson::Value &nonce)
{
    if (!nonce.IsString()) {
        throw std::runtime_error("invalid .subscribe response: extra nonce is not a string");
    }
    ...
    if (len & 1) {
        throw std::runtime_error("invalid .subscribe response: extra nonce has an odd number of hex chars");
    }

    if (len > 8) {
        throw std::runtime_error("Invalid .subscribe response: extra nonce is too long");
    }
...
const char *jdkcat::EthStratumClient::errorMessage(const rapidjson::Value &error)
{
...
void jdkcat::EthStratumClient::authorize()
{
    using namespace rapidjson;
    ...
    JsonRequest::create(doc, m_sequence, build_little_string("authorize").c_str(), params);

    send(doc, [this](const rapidjson::Value& result, bool success, uint64_t elapsed) { onAuthorizeResponse(result, success, elapsed); });
}

void jdkcat::EthStratumClient::onAuthorizeResponse(const rapidjson::Value &result, bool success, uint64_t)
{
    try {
        if (!success) {
            ...
            throw std::runtime_error(".authorize call failed");
        }

        if (!result.IsBool()) {
            throw std::runtime_error("invalid .authorize response: result is not a boolean");
        }
...
void jdkcat::EthStratumClient::onSubscribeResponse(const rapidjson::Value &result, bool success, uint64_t)
{
    if (!success) {
        return;
    }
    ...
    try {
        if (!result.IsArray()) {
            throw std::runtime_error("invalid .subscribe response: result is not an array");
        }

        auto arr = result.GetArray();

        if (arr.Size() <= 1) {
            throw std::runtime_error("invalid .subscribe response: result array is too short");
        }
...
#       endif
        ...
        if (m_pool.isNicehash()) {
            ...
            Document doc(kObjectType);
            Value params(kArrayType);
            JsonRequest::create(doc, m_sequence, build_little_string("extranonce.subscribe").c_str(), params);
            send(doc);
        }
    } catch (const std::exception &ex) {
...
void jdkcat::EthStratumClient::subscribe()
{
    using namespace rapidjson;
    ...
    JsonRequest::create(doc, m_sequence, build_little_string("subscribe").c_str(), params);
