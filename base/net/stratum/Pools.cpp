 ...
 * Copyright 2016-2021 jdkcat       <https://github.com/jdkcat>, <support@jdkcat.com>
 *
...
bool jdkcat::Pools::isEqual(const Pools &other) const
{
...
int jdkcat::Pools::donateLevel() const
{
#   ifdef jdkcat_FEATURE_BENCHMARK
...
jdkcat::IStrategy *jdkcat::Pools::createStrategy(IStrategyListener *listener) const
{
    if (active() == 1) {
...
rapidjson::Value jdkcat::Pools::toJSON(rapidjson::Document &doc) const
{
...
size_t jdkcat::Pools::active() const
{
    size_t count = 0;
    for (const Pool &pool : m_data) {
        if (pool.isEnabled()) {
...
void jdkcat::Pools::load(const IJsonReader &reader)
{
    m_data.clear();
...
uint32_t jdkcat::Pools::benchSize() const
{
#   ifdef jdkcat_FEATURE_BENCHMARK
...
void jdkcat::Pools::toJSON(rapidjson::Value &out, rapidjson::Document &doc) const
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
void jdkcat::Pools::setDonateLevel(int level)
{
...
void jdkcat::Pools::setProxyDonate(int value)
{
    switch (value) {
...
void jdkcat::Pools::setRetries(int retries)
{
...
void jdkcat::Pools::setRetryPause(int retryPause)
{
