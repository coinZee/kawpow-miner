...
jdkcat::DaemonClient::DaemonClient(int id, IClientListener *listener) :
...
jdkcat::DaemonClient::~DaemonClient()
{
    delete m_timer;
    delete m_ZMQSocket;
}
...
void jdkcat::DaemonClient::deleteLater()
{
...
bool jdkcat::DaemonClient::disconnect()
{
...
bool jdkcat::DaemonClient::isTLS() const
{
#   ifdef jdkcat_FEATURE_TLS
    return m_pool.isTLS();
#   else
...
int64_t jdkcat::DaemonClient::submit(const JobResult &result)
{
...
void jdkcat::DaemonClient::connect()
{
...
void jdkcat::DaemonClient::connect(const Pool &pool)
{
    setPool(pool);
    connect();
}
...
void jdkcat::DaemonClient::setPool(const Pool &pool)
{
...
void jdkcat::DaemonClient::onHttpData(const HttpData &data)
{
    if (data.status != 200) {
        return retry();
    }
...
void jdkcat::DaemonClient::onTimer(const Timer *)
{
    if (m_pool.zmq_port() >= 0) {
        m_prevHash = nullptr;
...
void jdkcat::DaemonClient::onResolved(const DnsRecords &records, int status, const char* error)
{
    m_dns.reset();
...
bool jdkcat::DaemonClient::isOutdated(uint64_t height, const char *hash) const
{
    return m_job.height() != height || m_prevHash != hash || Chrono::steadyMSecs() >= m_jobSteadyMs + m_pool.jobTimeout();
}


bool jdkcat::DaemonClient::parseJob(const rapidjson::Value &params, int *code)
{
...
bool jdkcat::DaemonClient::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == -1) {
        return false;
    }
...
int64_t jdkcat::DaemonClient::getBlockTemplate()
{
    using namespace rapidjson;
    Document doc(kObjectType);
...
int64_t jdkcat::DaemonClient::rpcSend(const rapidjson::Document &doc, const std::map<std::string, std::string> &headers)
{
...
void jdkcat::DaemonClient::retry()
{
    m_failures++;
...
void jdkcat::DaemonClient::send(const char *path)
{
...
void jdkcat::DaemonClient::setState(SocketState state)
{
    if (m_state == state) {
        return;
    }
...
void jdkcat::DaemonClient::onZMQConnect(uv_connect_t* req, int status)
{
...
void jdkcat::DaemonClient::onZMQRead(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf)
{
...
void jdkcat::DaemonClient::onZMQClose(uv_handle_t* handle)
{
...
void jdkcat::DaemonClient::onZMQShutdown(uv_handle_t* handle)
{
    DaemonClient* client = getClient(handle->data);
    if (client) {
#       ifdef APP_DEBUG
...
void jdkcat::DaemonClient::ZMQConnected()
{
#   ifdef APP_DEBUG
...
bool jdkcat::DaemonClient::ZMQWrite(const char* data, size_t size)
{
...
void jdkcat::DaemonClient::ZMQRead(ssize_t nread, const uv_buf_t* buf)
{
    if (nread <= 0) {
...
void jdkcat::DaemonClient::ZMQParse()
{
#   ifdef APP_DEBUG
    std::vector<char> msg;
#   endif
...
#   endif
    ...
    // Clear previous hash and check daemon height to guarantee that jdkcat will call get_block_template RPC later
...
bool jdkcat::DaemonClient::ZMQClose(bool shutdown)
{
    if ((m_ZMQConnectionState == ZMQ_NOT_CONNECTED) || (m_ZMQConnectionState == ZMQ_DISCONNECTING)) {
        if (shutdown) {
