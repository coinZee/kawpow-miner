...
jdkcat::HttpResponse::HttpResponse(uint64_t id, int statusCode) :
    m_id(id),
...
bool jdkcat::HttpResponse::isAlive() const
{
...
void jdkcat::HttpResponse::end(const char *data, size_t size)
{
    if (!isAlive()) {
        return;
    }
