...
jdkcat::HttpClient::HttpClient(const char *tag, FetchRequest &&req, const std::weak_ptr<IHttpListener> &listener) :
...
bool jdkcat::HttpClient::connect()
{
...
void jdkcat::HttpClient::onResolved(const DnsRecords &records, int status, const char *error)
{
    this->status = status;
    m_dns.reset();
...
void jdkcat::HttpClient::onTimer(const Timer *)
{
    close(UV_ETIMEDOUT);
}


void jdkcat::HttpClient::handshake()
{
...
void jdkcat::HttpClient::read(const char *data, size_t size)
{
    if (!parse(data, size)) {
        close(UV_EPROTO);
    }
...
void jdkcat::HttpClient::onConnect(uv_connect_t *req, int status)
{
