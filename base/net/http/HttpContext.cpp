...
{
public:
    jdkcat_DISABLE_COPY_MOVE_DEFAULT(HttpWriteBaton)
...
jdkcat::HttpContext::HttpContext(int parser_type, const std::weak_ptr<IHttpListener> &listener) :
    HttpData(SEQUENCE++),
...
jdkcat::HttpContext::~HttpContext()
{
    delete m_tcp;
    delete m_parser;
}
...
void jdkcat::HttpContext::write(std::string &&data, bool close)
{
...
bool jdkcat::HttpContext::isRequest() const
{
    return m_parser->type == HTTP_REQUEST;
}


bool jdkcat::HttpContext::parse(const char *data, size_t size)
{
    if (size == 0) {
        return true;
    }
...
std::string jdkcat::HttpContext::ip() const
{
    char ip[46]           = {};
...
uint64_t jdkcat::HttpContext::elapsed() const
{
    return Chrono::steadyMSecs() - m_timestamp;
}


void jdkcat::HttpContext::close(int status)
{
    if (!get(id())) {
        return;
    }
...
jdkcat::HttpContext *jdkcat::HttpContext::get(uint64_t id)
{
...
void jdkcat::HttpContext::closeAll()
{
    for (auto &kv : storage) {
...
int jdkcat::HttpContext::onHeaderField(llhttp_t *parser, const char *at, size_t length)
{
...
int jdkcat::HttpContext::onHeaderValue(llhttp_t *parser, const char *at, size_t length)
{
    auto ctx = static_cast<HttpContext*>(parser->data);
...
void jdkcat::HttpContext::attach(llhttp_settings_t *settings)
{
...
void jdkcat::HttpContext::setHeader()
{
