...
bool jdkcat::HttpData::isJSON() const
{
...
const char *jdkcat::HttpData::methodName() const
{
    return llhttp_method_name(static_cast<llhttp_method>(method));
}


rapidjson::Document jdkcat::HttpData::json() const
{
    if (status < 0) {
...
const char *jdkcat::HttpData::statusName(int status)
{
    if (status < 0) {
