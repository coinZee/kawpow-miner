...
jdkcat::HttpServer::HttpServer(const std::shared_ptr<IHttpListener> &listener) :
    m_listener(listener)
{
}


jdkcat::HttpServer::~HttpServer()
{
    HttpContext::closeAll();
}


void jdkcat::HttpServer::onConnection(uv_stream_t *stream, uint16_t)
{
