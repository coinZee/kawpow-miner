...
{
public:
    jdkcat_DISABLE_COPY_MOVE(LogPrivate)
...
void jdkcat::Log::add(ILogBackend *backend)
{
    assert(d != nullptr);
...
void jdkcat::Log::init()
{
    d = new LogPrivate();
}


void jdkcat::Log::print(const char *fmt, ...)
{
    if (!d) {
        return;
    }
...
void jdkcat::Log::print(Level level, const char *fmt, ...)
{
    if (!d) {
        return;
    }
