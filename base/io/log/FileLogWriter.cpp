...
jdkcat::FileLogWriter::FileLogWriter()
{
    init();
}

jdkcat::FileLogWriter::FileLogWriter(const char* fileName)
{
    init();
    open(fileName);
}
...
void jdkcat::FileLogWriter::init()
{
...
bool jdkcat::FileLogWriter::open(const char *fileName)
{
...
bool jdkcat::FileLogWriter::write(const char *data, size_t size)
{
    if (!isOpen()) {
        return false;
    }
...
bool jdkcat::FileLogWriter::writeLine(const char *data, size_t size)
{
    if (!isOpen()) {
        return false;
    }
...
void jdkcat::FileLogWriter::flush()
{
    uv_mutex_lock(&m_buffersLock);
