...
jdkcat::ConsoleLog::ConsoleLog(const Title &title)
{
    if (!isSupported()) {
        Log::setColors(false);
        return;
...
jdkcat::ConsoleLog::~ConsoleLog()
{
    Handle::close(m_tty);
}


void jdkcat::ConsoleLog::print(uint64_t, int, const char *line, size_t, size_t size, bool colors)
{
...
bool jdkcat::ConsoleLog::isSupported()
{
...
#ifdef jdkcat_OS_WIN
bool jdkcat::ConsoleLog::isWritable() const
{
