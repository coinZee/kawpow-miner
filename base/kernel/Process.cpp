...
#       endif
    ...
    if (location == Process::ExeLocation) {
        ...
        const auto pos  = path.rfind(*jdkcat_DIR_SEPARATOR);
...
jdkcat::Process::Process(int argc, char **argv) :
...
#   ifdef jdkcat_SHARED_DATADIR
    if (dataDir.empty()) {
        dataDir = fmt::format("{}" jdkcat_DIR_SEPARATOR ".jdkcat" jdkcat_DIR_SEPARATOR, location(HomeLocation));
        MKDIR(dataDir);
...
jdkcat::String jdkcat::Process::exepath()
{
    size_t size = sizeof(pathBuf);
...
jdkcat::String jdkcat::Process::location(Location location, const char *fileName)
{
    ...
    return fmt::format("{}" jdkcat_DIR_SEPARATOR "{}", path, fileName).c_str();
}
