...
char *jdkcat::Platform::createUserAgent()
{
...
#ifndef jdkcat_FEATURE_HWLOC
#ifdef __DragonFly__

bool jdkcat::Platform::setThreadAffinity(uint64_t cpu_id)
{
    return true;
}
...
#endif // __DragonFly__
#endif // jdkcat_FEATURE_HWLOC


void jdkcat::Platform::setProcessPriority(int)
{
}


void jdkcat::Platform::setThreadPriority(int priority)
{
    if (priority == -1) {
        return;
    }
...
bool jdkcat::Platform::isOnBatteryPower()
{
    for (int i = 0; i <= 1; ++i) {
        char buf[64];
...
uint64_t jdkcat::Platform::idleTime()
{
