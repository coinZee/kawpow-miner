...
void jdkcat::BaseTransform::load(JsonChain &chain, Process *process, IConfigTransform &transform)
{
...
void jdkcat::BaseTransform::finalize(rapidjson::Document &doc)
{
    using namespace rapidjson;
...
void jdkcat::BaseTransform::transform(rapidjson::Document &doc, int key, const char *arg)
{
    switch (key) {
...
#           ifdef jdkcat_FEATURE_TLS
                "stratum+ssl://randomx.jdkcat.com:443"
#           else
                "randomx.jdkcat.com:3333"
#           endif
            );
        } else
#       endif
...
void jdkcat::BaseTransform::transformBoolean(rapidjson::Document &doc, int key, bool enable)
{
    switch (key) {
    case IConfig::BackgroundKey: /* --background */
...
void jdkcat::BaseTransform::transformUint64(rapidjson::Document &doc, int key, uint64_t arg)
{
    switch (key) {
    case IConfig::RetriesKey: /* --retries */
